...
- Results saved to `artifacts/pubmed_AndreiCod.txt` containing article titles, author lists, and abstracts for downstream literature review.
...
- Saved copy as `artifacts/fasta_AndreiCod.fasta` for assignment deliverables.
...
## Task 4 — VCF → PubMed
- Linked VCF variants to PubMed literature per `Task4_vcf_pubmed.ipynb` using the following query formulation strategy:
  1. **Variants with rsID** (e.g., rs1042522): Search PubMed directly with the rsID as the query term, leveraging PubMed's automatic indexing of SNP identifiers.
  2. **Variants without rsID**: Search with the format `chr<CHROM>:<POS> AND TP53` to find position-specific literature while constraining to the gene of interest.
- The VCF contains the well-known TP53 Pro72Arg polymorphism (rs1042522) and R175H hotspot mutation (rs28934578), both with extensive cancer literature.
