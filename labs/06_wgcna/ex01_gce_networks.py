...
def create_synthetic_expression_data(
    output_path: Path, n_genes: int = 80, n_samples: int = 15
) -> pd.DataFrame:
    """
    ...
    pattern1 = np.concatenate(
        [np.linspace(20, 100, n_samples // 2), np.full(n_samples - n_samples // 2, 100)]
    )
    # Module 2: oscillating pattern 1
    pattern2 = 60 + 30 * np.sin(t)
    # Module 3: step function
    pattern3 = np.where(
        np.arange(n_samples) < n_samples // 3,
        30,
    ...
    patterns = [
        pattern1.astype(float),
        pattern2.astype(float),
        pattern3.astype(float),
        pattern4.astype(float),
    ]
